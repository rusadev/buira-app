...
export const LoginView: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Intelligent User & Tenant Detection
    const matchedUser = INITIAL_USER_ACCOUNTS.find(
    ...
    if (matchedUser) {
      ...
      // Fallback intelligent detection based on email domain/keywords
      if (cleanEmail.includes('geprek') || cleanEmail.includes('ayam')) {
        loginAsUser('user_ag_1');
      } else if (cleanEmail.includes('budi') || cleanEmail.includes('owner')) {
        loginAsUser('user_cs_1');
      } else if (cleanEmail.includes('superadmin') || cleanEmail.includes('ira')) {
        loginAsUser('user_superadmin_1');
      } else if (cleanEmail.includes('kopisenja') || cleanEmail.includes('barista') || cleanEmail.includes('coffee')) {
        loginAsUser('user_cs_1');
      } else {
        setErrorMsg('Email tidak terdaftar. Gunakan barista@kopisenja.id atau kasir@geprekmercon.id');
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-md p-8 space-y-6">
        ...
        {/* Subtle Demo Email Hints for Testing */}
        <div className="pt-4 border-t border-slate-100 text-center space-y-2">
          <span className="text-[11px] text-slate-400 font-medium block">Contoh Akun Uji Coba F&B:</span>
          <div className="flex flex-col gap-1 text-[11px]">
            <button
              onClick={() => { setEmail('barista@kopisenja.id'); setPassword('123456'); }}
              className="text-slate-600 hover:text-amber-600 font-semibold hover:underline"
            >
              ☕ Kopi Senja: <code className="bg-slate-100 px-1 py-0.5 rounded text-[10px]">barista@kopisenja.id</code>
            </button>
            <button
              onClick={() => { setEmail('kasir@geprekmercon.id'); setPassword('123456'); }}
              className="text-slate-600 hover:text-rose-600 font-semibold hover:underline"
            >
              🍗 Geprek Mercon: <code className="bg-slate-100 px-1 py-0.5 rounded text-[10px]">kasir@geprekmercon.id</code>
            </button>
            <button
              onClick={() => { setEmail('budi.owner@kopisenja.id'); setPassword('123456'); }}
              className="text-slate-600 hover:text-slate-900 font-semibold hover:underline"
            >
              👑 Owner (Multi-Outlet): <code className="bg-slate-100 px-1 py-0.5 rounded text-[10px]">budi.owner@kopisenja.id</code>
            </button>
          </div>
        </div>
      </div>
