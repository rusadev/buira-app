...
export const CashierView: React.FC = () => {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col md:flex-row min-w-0 overflow-hidden relative font-sans select-none">
      ...
      {/* Mobile 2-Tab Switcher Bar (Clean High-UX Full Width 2-Button Toggle) */}
      <div className="md:hidden bg-white p-2 border-b border-slate-200 shrink-0">
        <div className="grid grid-cols-2 gap-1.5 bg-slate-100 p-1 rounded-none border border-slate-200 w-full">
          <button
            type="button"
            onClick={() => setMobileTab('catalog')}
            className={`w-full py-2.5 px-3 rounded-none text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 ${
              mobileTab === 'catalog'
          ...
          <button
            ...
            className={`w-full py-2.5 px-3 rounded-none text-xs font-black uppercase tracking-wider transition-all flex items-center justify-center gap-2 relative ${
              mobileTab === 'cart'
                ? 'bg-red-600 text-white shadow-xs'
                : 'bg-white text-slate-700 hover:bg-slate-50 border border-slate-200'
            }`}
          ...
          >
            ...
            <span>Keranjang</span>
            {totalCartCount > 0 && (
              <span className={`px-1.5 py-0.2 rounded-none text-[10px] font-black ${
                mobileTab === 'cart' ? 'bg-white text-red-600' : 'bg-red-600 text-white'
              }`}>
                {totalCartCount}
              </span>
            )}
      ...
      <div className={`flex-1 min-h-0 flex flex-col min-w-0 ${mobileTab === 'cart' ? 'hidden md:flex' : 'flex'}`}>
        ...
        {cart.length > 0 && mobileTab === 'catalog' && (
          <div className="md:hidden p-3 pb-20 md:pb-3 bg-white border-t border-slate-200 shrink-0">
            <button
              type="button"
              onClick={() => setMobileTab('cart')}
